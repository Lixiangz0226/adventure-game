...
public class MapPresenter {
    ...
    private void drawSlotWindow() {
        gp.machine.fighting = true;
        gp.machine.getWindow().setVisible(true);
        gp.machine.runEvent();

        // Create a Swing Timer to check the shopOpened state periodically
        Timer timer = new Timer(20, e -> {
            if (!gp.machine.opened) {
                ((Timer) e.getSource()).stop(); // Stop the timer
                gp.machine.getWindow().setVisible(false);
            }
            else {
                gp.machine.getWindow().setVisible(true);
            }
